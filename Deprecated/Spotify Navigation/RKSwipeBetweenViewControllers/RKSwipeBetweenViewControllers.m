...
#define HEIGHT self.navigationBar.frame.size.height //%%% height of the segment
...
//    self.navigationBar.barTintColor = [UIColor colorWithRed:0.01 green:0.05 blue:0.06 alpha:1]; //%%% bartint
    ...
    UIViewController *sidewalkVC = [storyboard instantiateViewControllerWithIdentifier:@"Sidewalk VC"];
//    UIViewController *menuTableVC = [storyboard instantiateViewControllerWithIdentifier:@"Menu Table VC"];
    UIViewController *sideCalendarVC = [storyboard instantiateViewControllerWithIdentifier:@"Side Calendar List VC"];
    
    
    
    [viewControllerArray addObject:sidewalkVC];
    [viewControllerArray addObject:sideCalendarVC];
...
//    navigationView = [[UIView alloc]initWithFrame:CGRectMake(0,0, self.view.frame.size.width,self.navigationBar.frame.size.height)];
    navigationView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, self.navigationBar.frame.size.height)];
    NSInteger numControllers = [viewControllerArray count];
    if (!buttonText) {
         buttonText = [[NSArray alloc]initWithObjects: @"Sidewalk", @"Calendar", @"Third", nil]; //%%%buttontitle
    }
    
    for (int i = 0; i<numControllers; i++) {
//        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(X_BUFFER+i*(self.view.frame.size.width-2*X_BUFFER)/numControllers-X_OFFSET, Y_BUFFER,
//                                                                     (self.view.frame.size.width-2*X_BUFFER)/numControllers, HEIGHT)];

        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(X_BUFFER+i*(SCREEN_WIDTH-2*X_BUFFER)/numControllers-X_OFFSET, (self.navigationBar.frame.size.height - HEIGHT)/2.0,
                                                                     (SCREEN_WIDTH-2*X_BUFFER)/numControllers, HEIGHT)];

        [navigationView addSubview:button];
        
        button.tag = i; //%%% IMPORTANT: if you make your own custom buttons, you have to tag them appropriately
//        button.backgroundColor = [UIColor colorWithRed:0.03*i green:0.07 blue:0.08 alpha:1];//%%% buttoncolors
...
//    selectionBar = [[UIView alloc]initWithFrame:CGRectMake(X_BUFFER-X_OFFSET, SELECTOR_Y_BUFFER,(self.view.frame.size.width-2*X_BUFFER)/[viewControllerArray count], SELECTOR_HEIGHT)];
    selectionBar = [[UIView alloc]initWithFrame:CGRectMake(X_BUFFER-X_OFFSET, SELECTOR_Y_BUFFER,(SCREEN_WIDTH-2*X_BUFFER)/[viewControllerArray count], SELECTOR_HEIGHT)];
//    selectionBar.backgroundColor = [UIColor greenColor]; //%%% sbcolor
    selectionBar.backgroundColor = [UIColor blackColor];
