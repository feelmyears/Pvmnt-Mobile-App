...
- (instancetype)initWithFrame:(CGRect)frame andCategoryViews:(NSArray *)categoryViews sliderDirection:(SliderDirection)direction categorySelectionBlock:(categorySelected)block {
    if ((self = [super initWithFrame:frame])) {
        ...
        [self slideItemAtIndex:0 animated:NO];
    }
    return self;
}
...
- (void)categoryViewTapped:(UITapGestureRecognizer *)tap {
    ...
    if (self.categorySelectedBlock && !self.shouldAutoSelectScrolledCategory) {
        ...
        self.selectedView = [self.categoryViews objectAtIndex:tap.view.tag];
    }
}
...
{
    [self.categoryViews enumerateObjectsUsingBlock:^(UIView<PvmntCategorySliderViewItem> *itemView, NSUInteger idx, BOOL *stop) {
        CGFloat scrollViewCenterXOffset = CGRectGetMidX(self.scrollView.bounds);
        CGFloat itemViewCenterXOffset = CGRectGetMidX(itemView.frame);
        
        CGFloat ratio;
        if (CGRectGetMaxX(itemView.frame) < CGRectGetMinX(scrollView.bounds) || CGRectGetMinX(itemView.frame) > CGRectGetMaxX(scrollView.bounds)) {
            ratio = 0;
        } else {
            ratio = 1 - fabsf(itemViewCenterXOffset - scrollViewCenterXOffset)/CGRectGetWidth(scrollView.bounds);
        }
        
        ratio = ceilf(ratio * 1000)/1000;
        [self updateItemView:itemView withRatio:ratio];
    }];

}

- (void)updateItemView:(UIView<PvmntCategorySliderViewItem> *)itemView withRatio:(CGFloat)ratio {
